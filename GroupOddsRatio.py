...
class GroupOddsRatio(object):
    ...
    def read_annotation_file( self):
        ...
        with open( self.annotationFile, "r") as inFile:
            ...
            print("GroupOddsRatio.read_annotation_file: number of entries read: %s" % lineCounter )
            print("GroupOddsRatio.read_annotation_file: number of item with annotation: %s" % len( itemAnnotation) )
            print("GroupOddsRatio.read_annotation_file: number of annotations: %s" % len( groupItems) )
         
        for group in sorted(groupItems):
            print("GroupOddsRatio.read_annotation_file %s: %s" % (group, len( groupItems[ group]) ) )
    ...
    def read_external_files(self):
        ...
        print("GroupOddsRatio.read_external_files: number of lists read: %s" % len( externalLists) )
        
        for lst in sorted( externalLists):
            print("GroupOddsRatio.read_external_files %s: %s" % (lst, len( externalLists[ lst]) ) )
    ...
    def read_background_list(self):
        ...
        print("GroupOddsRatio.read_background_list: number of items in background: %s" % len( backgroundItems) )
        ...
        print("GroupOddsRatio.read_background_list: annotation items overlap with background: %s out of %s" % ( len( filteredAnnotationItems), len( annotationItems) ) )
        ...
        for lst in self.externalLists:
            ...
            print("GroupOddsRatio.read_background_list: %s items overlap with background: %s out of %s" % ( lst, len( filteredExternalLists[ lst]), len( self.externalLists[ lst]) ) )
    ...
    def calculate_odds_ratio(self):
        ...
        outFileTwo = open( self.outputFile.split( ".")[0] + "_two_sided.tsv", "w")
